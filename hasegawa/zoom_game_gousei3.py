...
def face_detect_trim(img):
    ...
    for face in faces:
        ...
        for (x, y) in landmark:
            ...
            cv2.rectangle(img,(np.min(landmark,axis=0)[1],np.max(landmark,axis=0)[1]),(np.min(landmark,axis=0)[0],np.max(landmark,axis=0)[0]),(0,255,0),3)
        break
    return img, pos, landmark

def mouth_detect_trim(img):
    # 顔検出
    img_gry = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    faces = face_detector(img_gry, 1)
    ...
    for face in faces:
        ...
        '''
        pts = np.array([landmark[0],landmark[1],landmark[2],landmark[3],landmark[4],landmark[5],landmark[6],landmark[7]])
        ...
        #print(face.top())
        img= img[np.min(Landmark,axis=0)[1]:np.max(Landmark,axis=0)[1],np.min(Landmark,axis=0)[0]:np.max(Landmark,axis=0)[0]]
...
def capture_trim():
    ...
    while(True):
        ...
        img,_,_,_ = face_detect_trim(img)
        mouth = mouth_detect_trim(img)
        #img = face_recog(img)
...
def main():
    ...
    #frame,_,_,_ = face_detect_trim(frame)
    moutH = mouth_detect_trim(frame)
    ...
    #_,_,landmark,_ = face_detect_trim(frame)

    frame = frame.swapaxes(0,1)
    frame = pygame.surfarray.make_surface(frame)
    #pygame.draw.polygon(frame, (255,255,255), [landmark[0],landmark[1],landmark[2],landmark[3],landmark[4],landmark[5],landmark[6],landmark[7]])
    #faces = faces.swapaxes(0,1)
    #faces = pygame.surfarray.make_surface(faces)
    print(landmark)
    if len(landmark[0]>0):
        landmark=[landmark[0],landmark[1],landmark[2],landmark[3],landmark[4],landmark[5],landmark[6],landmark[7]]
    #kuchi=np.min(landmark,axis=0)
    tate=np.min(landmark,axis=0)[1]
    tate=SCR_RECT.bottom-frame.get_height()+tate

    kuchi=(np.min(landmark,axis=0)[0],tate)

    if type(moutH) is np.ndarray:
        moutH = moutH.swapaxes(0,1)
        moutH = pygame.surfarray.make_surface(moutH)
        Mouth.image = moutH
    else:
        Mouth.image = frame
    Player.image = frame
    ...
    try:
        while True:
            ...
            frame = frame.swapaxes(0,1)
            frame = pygame.surfarray.make_surface(frame)
            #landmark = pygame.draw.polygon(frame, (255,255,255), [landmark[0],landmark[1],landmark[2],landmark[3],landmark[4],landmark[5],landmark[6],landmark[7]])
            #faces = faces.swapaxes(0,1)
            if type(moutH) is np.ndarray:
                moutH = moutH.swapaxes(0,1)
                #print(type(moutH))
                ...
                #print(player.rect.topleft, "kao")
                #print(np.min(landmark,axis=0),"kuchi")

                tate= np.min(landmark,axis=0)
                kuchi=(player.rect.left + np.min(landmark,axis=0)[0], 
                player.rect.top + np.min(landmark,axis=0)[1])
                #print(kuchi)
                
                #print(mouth.rect)
            #landmark = pygame.surfarray.make_surface(landmark)
            screen.blit(Back_image, back_rect)
            Player.image = frame

            clock.tick(40)
            all.update(kuchi)
            if type(moutH) is pygame.Surface:
                collision_detection(mouth, aliens, beams)
            all.draw(screen)
        ...
        pygame.quit()
        cv2.destroyAllWindows()
def collision_detection(mouth, aliens, beams):
    """衝突判定"""
    # プレイヤーとビームの衝突判定
    pygame.sprite.spritecollide(mouth, beams, True)

class Mouth(pygame.sprite.Sprite):
    speed = 50  # 移動速度
    reload_time = 15  # リロード時間
    def __init__(self,kuchi):
        pygame.sprite.Sprite.__init__(self, self.containers)
        self.rect = self.image.get_rect()

        self.rect.topleft = kuchi

        self.reload_timer = 0
    def update(self,kuchi):
        # 押されているキーをチェック
